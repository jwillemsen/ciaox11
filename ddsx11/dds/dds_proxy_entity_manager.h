...
namespace DDSX11
{
  ...
  private:
    ...
    /// Map containing all Topic C++11 proxies. For a specific native DDS Topic there can multiple
    /// instances using the same instance handle but different points, this because of the functionality
    /// of find_topic which creates a new instance each time.
    typedef std::map<DDS_Native::DDS::Entity *, IDL::traits< ::DDS::Topic>::ref_type> TopicProxies;
